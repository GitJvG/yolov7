...
def load_model(weights = ROOT / 'weights/best.pt',  # weight.pt path(s)
               device='',  # cuda device, i.e. 0 or 0,1,2,3 or cpu
               dnn=False, # use OpenCV DNN for ONNX inference
               data=ROOT / 'data/coco128.yaml',  # dataset.yaml path
               half=False, # use FP16 half-precision inference
               update=False # update model (to fix SourceChangeWarning)
               ):
            ...
            if len(det):
                ...
                if split_no_det:
                    if dataset.mode == 'image':
                        ...
                        filename_no_det = file_path.stem + '_no_det' + file_path.suffix
                        save_path = str(file_path.parent / filename_no_det)
